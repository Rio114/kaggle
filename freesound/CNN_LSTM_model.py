...
def CNN_LSTM_Model(num_freq = 128, len_div = 256):
    ...
    conv1 = Conv2D(n_filter, (3, 1),activation='relu',padding='same',name='conv1')(inputs)
    pool1 = MaxPooling2D((3, 1),strides=(2, 1),padding='same',name='pool1')(conv1)
    norm1 = BatchNormalization(axis=-1, name='norm1')(pool1)
    reshape1 = Reshape([-1, len_div])(norm1)
    conv_list.append(reshape1)

    conv2 = Conv2D(n_filter, (3, 1),activation='relu',padding='same',name='conv2')(norm1)
    pool2 = MaxPooling2D((3, 1),strides=(2, 1),padding='same',name='pool2')(conv2)
    norm2 = BatchNormalization(axis=-1, name='norm2')(pool2)
    reshape2 = Reshape([-1, len_div])(norm2)
    conv_list.append(reshape2)

    conv3 = Conv2D(n_filter, (3, 1),activation='relu',padding='same',name='conv3')(norm2)
    pool3 = MaxPooling2D((3, 1),strides=(2, 1),padding='same',name='pool3')(conv3)
